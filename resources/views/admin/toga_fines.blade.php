...
@section('content')
                ...
                <tbody>
                    @foreach ($borrowers as $borrower)
                        <tr>
                            ...
                            <td>{{ \Carbon\Carbon::parse($borrower->date_issued)->format('m-d-Y') }}</td>
                            <td>{{ \Carbon\Carbon::parse($borrower->expected_date_returned)->format('m-d-Y') }}</td>                            
                            <td>
                                <button type="button" class="btn btn-danger btn-sm rounded-circle view_btn"
                                    data-id="{{ $borrower->id }}" data-name="{{ $borrower->student_name }}"
                                    data-items="{{ $borrower->item_names }}" data-quantity="{{ $borrower->quantity }}"
                                    data-bs-toggle="modal" data-bs-target="#returnConfirmationModal">
    ...
    <!-- Add Borrower Modal -->
    <div class="modal fade" id="addBorrowerModal" tabindex="-1" aria-labelledby="addBorrowerModalLabel"
        ...
        <div class="modal-dialog modal-lg"> <!-- Use modal-lg to make it larger -->
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p class="text-danger fst-italic">Note: If the return date exceeds the expected date, a fee of 20
                        pesos per item per day will be charged.</p>
                    ...
                    <form action="{{ route('admin.addBorrower') }}" method="POST">
                        ...
                        <div class="row">
                                        ...
                                        @foreach ($items as $item)
                                            <div class="form-check">
                                                <input class="form-check-input item-checkbox" type="checkbox"
                                                    id="item_{{ $item->id }}" name="item_ids[]"
                                                    value="{{ $item->id }}"
                                                    {{ in_array($item->id, old('item_ids', [])) ? 'checked' : '' }}>
                                                <label class="form-check-label" for="item_{{ $item->id }}">
                                                    {{ $item->item_name }} (Available:
                                                    {{ $item->total_quantity - $item->quantity_borrowed }})
                                                </label>
                                            </div>
                                        @endforeach
                                    </div>
                            ...
                            <div class="col-md-5">
                                ...
                                <div class="mb-3">
                                    <label for="expectedDateReturned" class="form-label">Expected Return
                                        Date</label>
                                    <input type="date" class="form-control" id="expectedDateReturned"
                                        name="expected_date_returned" value="{{ old('expected_date_returned') }}"
                                        required>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            returnModal.addEventListener('show.bs.modal', function(event) {
                ...
                const borrowerId = button.getAttribute('data-id');
                const borrowerName = button.getAttribute('data-name');
                const itemNames = button.getAttribute('data-items').split(',');
                const quantity = button.getAttribute('data-quantity');

                // Update the modal's content
                borrowerNameSpan.textContent = borrowerName;

                // Clear previous content and add new items
                itemConditionsContainer.innerHTML = '';
                itemNames.forEach((itemName, index) => {
                    const itemHtml = `
                <div class="d-flex align-items-center mb-3">
                    <div class="form-check me-3">
                        <input class="form-check-input" type="checkbox" id="item_${index}" name="items[${index}][returned]" value="${itemName.trim()}">
                        <label class="form-check-label" for="item_${index}">
                            ${itemName.trim()} (Qty: ${quantity})
                        </label>
                    </div>
            ...
            `;
                    itemConditionsContainer.innerHTML += itemHtml;
                });
                ...
                itemConditionsContainer.querySelectorAll('.form-check-input').forEach((checkbox, idx) => {
                    checkbox.addEventListener('change', function() {
                        const dropdown = itemConditionsContainer.querySelectorAll(
                            '.form-select')[idx];
